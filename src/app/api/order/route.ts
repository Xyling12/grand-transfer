...
export async function POST(req: Request) {
    try {
        ...
        try {
            ...
            const sourceSite = origin.includes('taximezhgorod') ? 'taximezhgorod777.ru' : 'грандтрансфер.com';
        ...
        const host = req.headers.get('host') || 'unknown';
        const origin = req.headers.get('origin') || host;
        body.sourceSite = origin.includes('taximezhgorod') ? 'taximezhgorod777.ru' : 'грандтрансфер.com';
