...
export default function UserDetailModal({ isOpen, onClose, data, type, onUpdateFeedback, onOpenOrder }: UserDetailModalProps) {
    ...
    const handleAdminAction = async (action: 'status' | 'role' | 'delete', val: string) => {
        if (!confirm(`Вы уверены, что хотите выполнить это действие: ${action} -> ${val}?`)) return;
        setIsLoadingAction(true);
        try {
            if (action === 'delete') {
                const res = await fetch(`/api/admin/users?id=${data.id}`, { method: 'DELETE' });
                if (res.ok) { alert('Пользователь удален. Обновите страницу.'); onClose(); }
                else alert('Ошибка: возможно есть зависшие заказы.');
            } else {
                const body = action === 'status' ? { id: data.id, status: val } : { id: data.id, role: val };
                const res = await fetch(`/api/admin/users`, {
                    method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body)
                });
                if (res.ok) { alert('Успешно обновлено. Обновите страницу для применения изменений.'); onClose(); }
            }
        } catch (e) {
            ...
            <div style={{ marginTop: '1.5rem', paddingRight: '0.25rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {sortedOrders.map((o: any) => {
                        ...
                        const orderLink = `/admin/orders?orderId=${o.id}`;
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyItems: 'center', justifyContent: 'center', padding: '1rem' }}>
            ...
            <div style={{
                        ...
                        <div>
                            ...
                            <div style={{ color: 'var(--color-text-muted)', fontSize: '0.875rem', marginTop: '0.25rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                                    ...
                                    {data.phone ? <a href={`tel:+${data.phone.replace(/[^0-9]/g, '')}`} style={{ color: 'inherit', textDecoration: 'none' }} onClick={(e) => e.stopPropagation()}>{data.phone}</a> : 'Нет телефона'}
                                </span>
                                {data.username && (
                                    <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', color: '#60a5fa' }}>
                                        ...
                                        <a href={`https://t.me/${data.username}`} target="_blank" rel="noopener noreferrer" style={{ color: 'inherit', textDecoration: 'none' }} onClick={(e) => e.stopPropagation()}>{data.username}</a>
                                    </span>
                                )}
                            </div>
                ...
                <div style={{ flex: 1, padding: '1.5rem', overflowY: 'auto' }}>
                    ...
                    {/* Admin Controls Area */}
                    {type !== 'client' && (
                        <div style={{ marginBottom: '1.5rem', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '0.75rem', padding: '1rem' }}>
