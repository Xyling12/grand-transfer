...
export function registerAdminHandlers(deps: BotDeps) {
    ...
    const handleHelp = async (ctx: any) => {
        ...
        let msg = `🤖 <b>Справка по боту GrandTransfer (v1.6.1)</b>\n\n`;
        ...
        msg += `\n📌 <b>Что нового (v1.6.1):</b>\n`;
        msg += `- 🎫 Система тикетов: двусторонний диалог с поддержкой\n`;
        msg += `- 🐛 Баг-репорты: отдельная система отслеживания ошибок\n`;
        msg += `- 📩 Мои обращения: история всех ваших тикетов\n`;
        msg += `- 🗺 Маршрут: ссылки на Яндекс Карты (приложение + браузер)\n`;
...
**Что нового (1.6.1):**
- 🐛 Исправлена безопасность /unban (ре-верификация)
- 🛡 Исправлена модерация ссылок в группе
- ⚠️ Подтверждение при очистке БД

**Что нового (1.6.0):**
- 🎫 Двусторонняя система тикетов поддержки
- 🐛 Баг-репорты для администраторов
- 📩 «Мои обращения» для всех ролей
- 🗺 Dual-link маршруты (приложение + браузер)
- 🛡 Управление защитой контента
- ⌨️ Автовосстановление меню-клавиатуры
`;
