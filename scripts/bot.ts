...
const handleHelp = async (ctx: any) => {
    ...
    let msg = `🤖 <b>Справка по боту GrandTransfer (v1.3.4)</b>\n\n`;
    ...
    msg += `📌 <b>Обновление v1.3.6 (${new Date().toLocaleDateString('ru-RU')}):</b>\n`;
    msg += `- 👨‍💻 **Медиа в CRM**: Изображения ПТС и СТС водителей теперь можно напрямую открывать из веб-панели.\n`;
...
**Что нового:**
- Многошаговая регистрация (Сбор ФИО, ПТС, СТС, фото авто)
- Медиа-Шлюз Telegram для выгрузки фото в CRM
- Новые команды Администратора (/add_driver, /approve, /reject, /ban, /unban)
- Интеграция кнопки "CRM Система" с Telegram Auth Widget
- Дашборд "Доска Заказов" для отслеживания связей (Клиент ➡️ Диспетчер ➡️ Водитель)
- Очистка системных сообщений в чате
- Привязка общей БД (SQLite) для двух сайтов
            `;
