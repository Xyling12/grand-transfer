...
bot.hears('ℹ️ Справка', async (ctx) => {
    ...
    let msg = `🤖 <b>Справка по боту GrandTransfer (v1.3.2)</b>\n\n`;
    ...
    msg += `📌 <b>Обновление v1.3.2 (25.02.2026):</b>\n`;
    msg += `- 🚀 **Миграция сервера**: Успешный переезд на надежный облачный сервер Timeweb.\n`;
    msg += `- ⚡ **Сборка через GitHub**: Сайт теперь не виснет при обновлении, сборка идет на мощностях Microsoft Github (CI/CD Pipeline).\n`;
    msg += `- 📄 **Полная заявка**: Добавлены кнопки "Полная заявка" прямо в список активных заявок.\n`;
    msg += `- 👨‍✈️ **Роли**: Строгое разделение видимости кнопок для диспетчеров и водителей.\n`;
...
bot.command('version', async (ctx) => {
    ctx.reply('🤖 GrandTransfer Bot v1.3.2\n\n📌 Обновление v1.3.2:\n- 🚀 Успешный переезд на сервер Timeweb.\n- ⚡ Сборка перенесена на GitHub Actions для скорости.\n- 📄 Добавлены кнопки "Полная заявка" в активных заказах.\n- 👨‍✈️ Мои Заявки для диспетчеров и строгое ролевое меню.\n\nv1.3.1:\n- Разделение кнопок Диспетчеров и Водителей.\n- Защита от Docker crash-loop.', { protect_content: true });
});
